...
describe("NFTMarketplace", function() {
    ...
    it("Auction: create successfully", async function(){
        await nft.connect(seller).mint("uri",{value: ethers.parseEther("0.01")});
        ...
        await marketplace.connect(seller).createAuction(
            await nft.getAddress(),1,ethers.parseEther("0.1"),2
        );
    ...
    it("Bid:normal bid success", async function(){
        await nft.connect(seller).mint("uri",{value:ethers.parseEther("0.01")});
        await nft.connect(seller).setApprovalForAll(await marketplace.getAddress(),true);
        await marketplace.connect(seller).createAuction(await nft.getAddress(),1,ethers.parseEther("0.1"),2);

        await marketplace.connect(bidder1).placeBid(1,{value:ethers.parseEther("0.12")});
        const auc = await marketplace.getAuction(1);
    ...
    it("Bid:bid too low reverted", async function(){
        ...
        await expect(marketplace.connect(bidder1).placeBid(1,{value:ethers.parseEther("0.05")}))
    ...
    it("withdraw: get refund back",async function(){
        ...
        await marketplace.connect(bidder1).placeBid(1,{value:ethers.parseEther("0.12")});
        await marketplace.connect(bidder2).placeBid(1,{value:ethers.parseEther("0.13")});
    ...
    it("Utils:ethToUsd works", async function(){
        ...
        await nft.connect(seller).setApprovalForAll(await marketplace.getAddress(),true);
        await marketplace.connect(seller).createAuction(await nft.getAddress(),1,ethers.parseEther("0.1"),2);
        await marketplace.connect(bidder1).placeBid(1,{value:ethers.parseEther("0.1")});
